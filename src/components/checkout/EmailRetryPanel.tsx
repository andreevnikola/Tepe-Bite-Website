...
const T = {
  bg: {
    ...
    orderLabel: "Поръчка №",
    pendingWarning:
      "Поръчката НЕ е изпратена за обработка — трябва първо да се потвърди.",
    ...
    sending: "Изпращане...",
    retrySuccess:
      "Изпратено! Провери пощата и кликни линка, за да активираш поръчката.",
    failedAgain: "Не успяхме да изпратим имейла. Пишете ни на",
    failedSuffix: (orderNum: string) =>
      ` с номер на поръчката ${orderNum} и ще я потвърдим ръчно.`,
  ...
  en: {
    ...
    orderLabel: "Order #",
    pendingWarning:
      "Your order is NOT being processed — it must be confirmed first.",
    ...
    sending: "Sending...",
    retrySuccess:
      "Sent! Check your inbox and click the link to activate your order.",
    failedAgain: "Failed to send. Please write to us at",
    failedSuffix: (orderNum: string) =>
      ` with order number ${orderNum} and we will confirm it manually.`,
...
export default function EmailRetryPanel({
  ...
  const [retryState, setRetryState] = useState<
    "idle" | "sending" | "success" | "failed" | "expired"
  >("idle");
        ...
        {retryState === "success" ? (
          ...
          >
            ...
            {retryState === "expired"
              ...
              : t.failedSuffix(publicOrderNumber)}
          </div>
        ) : (
          <Fragment>
            <button
