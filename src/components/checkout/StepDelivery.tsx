...
export default function StepDelivery({
  ...
  const officeTotal = freeBase
    ? officeSurchargeCents
    : baseCents + officeSurchargeCents;
  const addressTotal = freeBase
    ? addressSurchargeCents
    : baseCents + addressSurchargeCents;

  function priceLabel(method: "speedy_locker" | "speedy_office" | "address"): {
    text: string;
    isFree: boolean;
    isSurcharge: boolean;
  } {
    if (method === "speedy_locker") {
      if (lockerTotal === 0)
        return { text: t.free, isFree: true, isSurcharge: false };
      return {
        text: baseCents > 0 ? formatMoneyEUR(baseCents) : t.free,
        isFree: false,
        isSurcharge: false,
      };
    }
        ...
        return {
          text:
            officeSurchargeCents > 0
              ? `+${formatMoneyEUR(officeSurchargeCents)}`
              : t.free,
          isFree: officeSurchargeCents === 0,
          isSurcharge: true,
        };
      return {
        text: officeTotal > 0 ? formatMoneyEUR(officeTotal) : t.free,
        isFree: false,
        isSurcharge: false,
      };
    }
      ...
      return {
        text:
          addressSurchargeCents > 0
            ? `+${formatMoneyEUR(addressSurchargeCents)}`
            : t.free,
        isFree: addressSurchargeCents === 0,
        isSurcharge: true,
      };
    return {
      text: addressTotal > 0 ? formatMoneyEUR(addressTotal) : t.free,
      isFree: false,
      isSurcharge: false,
    };
  }
      ...
      {threshold > 0 && (
        ...
        >
          ...
          <span>
            {freeBase ? (
              lang === "bg" ? (
                "Безплатна доставка до Speedy автомат включена!"
              ) : (
                "Free locker delivery included!"
              )
            ) : (
              <>
                  ...
                  <div>
                    <div
                      style={{
                        fontWeight: 700,
                        fontSize: "0.95rem",
                        color: active ? "var(--plum)" : "var(--text)",
                      }}
                    >
                      {m.label}
                    </div>
      ...
      {delivery.method === "address" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            >
              {t.cityLabel}{" "}
              <span style={{ color: "oklch(55% 0.18 20)" }}>*</span>
            </label>
            <input
              value={delivery.city}
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <input
                ...
                onChange={(e) =>
                  onChange({ ...delivery, street: e.target.value })
                }
            ...
            >
              ...
              <input
                ...
                onChange={(e) =>
                  onChange({ ...delivery, postalCode: e.target.value })
                }
                placeholder="4000"
                style={fieldStyle}
              />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <input
              ...
              onChange={(e) =>
                onChange({ ...delivery, building: e.target.value })
              }
